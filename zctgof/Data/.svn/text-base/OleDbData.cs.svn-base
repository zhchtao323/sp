...
{  
    ...
    /// ms  sql 数据库操作类
    /// </summary>
    public class OLeDbData : IDisposable, IData //IData,
    { 
        private OleDbConnection sqlcon;
        private OleDbTransaction sqlt;
        public OLeDbData(string conn)
        {
            sqlcon = new OleDbConnection(conn);
            sqlcon.Open();
        }
        ~OLeDbData()
        {
        ...
        {
            OleDbTransaction OleDbTransaction = this.sqlcon.BeginTransaction();
            foreach (String Sql1 in sql)
            {
                OleDbCommand OleDbCommand = new OleDbCommand(Sql1, this.sqlcon, OleDbTransaction);
        ...
        {
             sqlcon=new OleDbConnection(file);
        }
        ...
        public System.Data.IDataReader GetDr(string M_str_sqlstr)
        {
            ...
            return sqlcom.ExecuteReader(CommandBehavior.Default);
            //return sqlread;  
        }
        ...
        public void UpdateDs(System.Data.DataTable dt, string selsql)
        {
            ...
            OleDbDataAdapter sqld = new OleDbDataAdapter(selsql,sqlcon);
            OleDbCommandBuilder sqlb_zb = new OleDbCommandBuilder(sqld);
