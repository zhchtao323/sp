namespace Tiger.Video.VFW
{
	...
	public class AVIWriter : IDisposable
	{
        ...
        private int fcc = 541215044;//无压缩   mpeg4是842297453
		...
		public void Open(string fname, int width, int height)
		{
           ...
           if (Win32.AVIFileOpen(out file, fname, Win32.OpenFileMode.Create | Win32.OpenFileMode.Write, IntPtr.Zero) != 0)
			...
			this.height = height;
			// describe new stream			
            info = new Win32.AVISTREAMINFO();
			info.fccType	= Win32.mmioFOURCC("vids");
            //info.fccHandler = 541215044;//无 541215044  mpeg4是842297453 Win32.mmioFOURCC(codec);
            info.fccHandler = fcc; //Win32.mmioFOURCC(codec);
			info.dwScale	= 1;
			info.dwRate		= rate;
			...
			// describe compression options
			 opts = new Win32.AVICOMPRESSOPTIONS();
             opts.fccHandler = fcc;//  mpeg4 880309860;// Win32.mmioFOURCC(codec);
			opts.dwQuality	= quality;
            opts.dwFlags = 8;            
            //调用AVISaveOptions函数来显示一个对话框，可以让用户来选择压缩方式。
        ...
        public void Open2(string fname, int width, int height)
        {
            // close previous file
            Close();
            // calculate stride
        ...
        public void Open22(string fname, int width, int height)
        {
            // close previous file
            Close();
            // calculate stride
            ...
            // create new file
            //	
            //     if (Win32.AVIFileOpen(out file, fname,  Win32.OpenFileMode.Write, IntPtr.Zero) != 0)
            if (Win32.AVIFileOpen(out file, fname, Win32.OpenFileMode.Create | Win32.OpenFileMode.Write, IntPtr.Zero) != 0)
                throw new ApplicationException("Failed opening file");
            ...
            if (Win32.AVIFileCreateStream(file, out stream, ref info) != 0)
                throw new ApplicationException("Failed creating stream");
            //// describe compression options
            //Win32.AVICOMPRESSOPTIONS 
            opts = new Win32.AVICOMPRESSOPTIONS();
            opts.fccHandler = fcc;// Win32.mmioFOURCC(codec);
            opts.dwQuality = quality;
            opts.dwFlags = 8;           
         //   Win32.AVISaveOptions(stream, ref opts, IntPtr.Zero);
        ...
        // Add new frame to the AVI file
        public void AddFrame(System.Drawing.Bitmap bmp,int position1)
        {
            // check image dimension
            if ((bmp.Width != width) || (bmp.Height != height))
                throw new ApplicationException("Invalid image dimension");
            ...
            // write to stream
            if (Win32.AVIStreamWrite(streamCompressed, position1, 1, buf,
                stride * height, 0, IntPtr.Zero, IntPtr.Zero) != 0)
                throw new ApplicationException("Failed adding frame");
